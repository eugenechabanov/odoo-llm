...
class MISPeriodConfig(BaseModel):
    ...
    name: str = Field(..., description="Label for the period")
    mode: str = Field(
        default="fix",
        description="""
    ...
    date_from: str | None = Field(
        None, description="Start date for fixed period (YYYY-MM-DD)"
    )
    date_to: str | None = Field(
        None, description="End date for fixed period (YYYY-MM-DD)"
    )
    offset: int | None = Field(
        None, description="Offset from current period for relative mode"
    )
    duration: int | None = Field(
        None, description="Number of periods for relative mode"
    )
    type: str | None = Field(
        None,
        description="""
...
class MISInstanceConfig(BaseModel):
    ...
    name: str = Field(..., description="Name of the report instance")
    template_id: int = Field(..., description="ID of the MIS report template to use")
    date: str | None = Field(None, description="Base date for the report (YYYY-MM-DD)")
    ...
    company_id: int | None = Field(
        None, description="Company ID (leave empty for multi-company)"
    )
    multi_company: bool = Field(default=False, description="Enable multi-company mode")
    company_ids: list[int] | None = Field(
        None, description="List of company IDs for multi-company mode"
    )
    currency_id: int | None = Field(None, description="Currency ID for the report")
    periods: list[MISPeriodConfig] = Field(
        ..., description="List of periods to include in the report"
    )
    analytic_domain: str = Field(
        default="[]", description="Domain to filter analytic entries"
    )
...
class MISReportInstanceGenTool(BaseTool):
    ...
    def _run(
        self, name: str, template_id: int, periods: list[dict[str, Any]], **kwargs: Any
    ) -> dict[str, Any]:
