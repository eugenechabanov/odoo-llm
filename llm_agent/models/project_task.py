...
class ProjectTask(models.Model):
    ...
    def _has_ai_agent_assigned(self):
        ...
        self.ensure_one()
        return bool(
            self.env["llm.agent"].search_count(
    ...
    def _get_agent(self):
        ...
        return self.env["llm.agent"].search(
            [("user_id", "in", self.user_ids.ids), ("active", "=", True)], limit=1
        )
    ...
    def action_execute_ai_task(self):
        ...
        if not self.is_ai_executable:
            raise UserError(
                _(
                    "This task cannot be executed by AI. Please ensure it is assigned to an AI agent."
                )
            )
                ...
                for subtask in self.child_ids.sorted(lambda t: t.sequence):
                    ...
                    tasks.append(
                        Task(
                            ...
                            or f"Subtask of {self.name}",
                            expected_output=subtask.expected_output
                            or "Complete the subtask successfully",
    ...
    def _prepare_crew_kwargs(self, tasks, agents_to_use, agent):
        ...
        if self.process == "hierarchical":
            if not agent.member_ids or not agent.is_manager:
                raise UserError(
                    _("Hierarchical process requires a manager agent with team members")
                )
            ...
            crew_kwargs.update(
                {
                    ...
                    "agents": [
                        member._to_crewai_agent() for member in agent.member_ids
                    ],
                }
            )
        else:
    ...
    def _create_result_message(self, execution_time, result):
        ...
        message = "<b>AI Task Completed</b><br/>"
