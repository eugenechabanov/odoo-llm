...
export class LLMChatThreadRelatedRecord extends Component {
    ...
    async _getAvailableModels() {
        try {
            ...
            const commonModels = [
                ...
                "account.move", "project.project", "project.task", "crm.lead",
                "helpdesk.ticket", "hr.employee", "product.product", "product.template",
                "stock.picking", "mrp.production", "maintenance.request"
            ];
    ...
    _setupRecordPickerEventHandlers(modalElement, modal) {
        ...
        const selectedRecordInfo = modalElement.querySelector('.selected-record-info');
    ...
    async _searchRecords(model, query, resultsContainer, loadingContainer) {
        ...
        try {
            ...
            // Render results
            const resultsHtml = result.map(([id, name]) => `
      ...
      `).join('');
            ...
            resultsContainer.querySelectorAll('.record-item').forEach(item => {
                item.addEventListener('click', () => {
                    ...
                    resultsContainer.querySelectorAll('.record-item').forEach(i =>
    ...
    async _showUnlinkConfirmationDialog() {
        return new Promise((resolve) => {
            ...
            const recordName = this.relatedRecordDisplayName ||
