...
class LLMCrewAgent(models.Model):
    ...
    def _to_crewai_agent(self):
        return Agent(
            ...
            step_callback=lambda step: (
                self.env['crm.team'].search([('member_ids', 'in', [self.user_id.id])], limit=1).message_post(
    ...
    def _generate_step_message(self, step):
        ...
        html_parts = [
            ...
            f"<b>Output:</b> {output}" if output else (f"<b>Result:</b> {result}" if result else str(step))
        ]
