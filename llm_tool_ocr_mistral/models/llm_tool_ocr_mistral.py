...
class LLMToolOCRMistral(models.Model):
    """Mistral OCR tool for extracting text from PDFs and images"""

    _inherit = "llm.tool"

    @api.model
    def _get_available_implementations(self):
        """Register OCR tool implementation"""
        implementations = super()._get_available_implementations()
        return implementations + [("llm_tool_ocr_mistral", "Mistral OCR")]

    def llm_tool_ocr_mistral_execute(self, attachment_ids: List[int]) -> List[dict]:
        """
        ...
        Args:
            attachment_ids (list): List of attachment IDs to parse

        Returns:
            list: List of dicts, one per attachment: [{
                "attachment_id": int,
        ...
        Examples:
            >>> # Single attachment
            >>> tool.llm_tool_ocr_mistral_execute([123])
            [{
                "attachment_id": 123,
            ...
            >>> tool.llm_tool_ocr_mistral_execute([123, 124, 125])
            [
