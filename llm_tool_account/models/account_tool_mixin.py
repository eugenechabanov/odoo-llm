...
class AccountToolMixin(models.AbstractModel):
    ...
    def _read_group_compat(self, model, domain, groupby, aggregates):
        """Compatibility wrapper: Odoo 18 _read_group interface on Odoo 16.

        Converts Odoo 16 read_group dict results to Odoo 18 tuple format:
        (*groupby_record_values, *aggregate_values)
        """
        # Parse aggregate field names
        agg_fields = []
        for agg in aggregates:
            if agg == "__count":
                agg_fields.append(None)
            else:
                agg_fields.append(agg.split(":")[0])

        fields_param = [f for f in agg_fields if f is not None]

        groups = model.read_group(domain, fields=fields_param, groupby=groupby)

        results = []
        for group in groups:
            row = []
            # Groupby values: browse Many2one tuples back to records
            for gb_field in groupby:
                val = group.get(gb_field)
                if isinstance(val, (tuple, list)) and len(val) == 2:
                    comodel = model._fields[gb_field].comodel_name
                    row.append(self.env[comodel].browse(val[0]))
                elif (
                    val is False
                    and gb_field in model._fields
                    and model._fields[gb_field].type == "many2one"
                ):
                    comodel = model._fields[gb_field].comodel_name
                    row.append(self.env[comodel])
                else:
                    row.append(val)
            ...
            for agg in aggregates:
                if agg == "__count":
                    count_key = f"{groupby[0]}_count" if groupby else "__count"
                    row.append(group.get(count_key, 0))
                else:
                    field_name = agg.split(":")[0]
                    row.append(group.get(field_name, 0.0))
