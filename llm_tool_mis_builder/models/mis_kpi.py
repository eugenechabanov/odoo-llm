...
class MisReportKpi(models.Model):
    ...
    def mis_kpi_update(
        ...
        values = self._build_kpi_update_values(
            description=description,
            expression=expression,
            kpi_type=kpi_type,
            compare_method=compare_method,
    ...
    def _validate_choice(self, value, valid_options, field_name):
        """Validate that a value is in the list of valid options."""
        if value not in valid_options:
            raise UserError(
                _("Invalid %s '%s'. Must be one of: %s")
                % (field_name, value, ", ".join(valid_options))
            )

    def _build_kpi_update_values(self, **kwargs):
        """Build write values dict from optional update kwargs."""
        values = {}
        if kwargs.get("description") is not None:
            values["description"] = kwargs["description"]
        if kwargs.get("expression") is not None:
            values["expression"] = kwargs["expression"]
        if kwargs.get("kpi_type") is not None:
            self._validate_choice(kwargs["kpi_type"], ["num", "pct", "str"], "KPI type")
            values["type"] = kwargs["kpi_type"]
        if kwargs.get("compare_method") is not None:
            self._validate_choice(
                kwargs["compare_method"], ["diff", "pct", "none"], "compare_method"
            )
            values["compare_method"] = kwargs["compare_method"]
        if kwargs.get("accumulation_method") is not None:
            self._validate_choice(
                kwargs["accumulation_method"],
                ["sum", "avg", "none"],
                "accumulation_method",
            )
            values["accumulation_method"] = kwargs["accumulation_method"]
        if kwargs.get("sequence") is not None:
            values["sequence"] = kwargs["sequence"]
        if kwargs.get("auto_expand_accounts") is not None:
            values["auto_expand_accounts"] = kwargs["auto_expand_accounts"]
        if kwargs.get("style_id") is not None:
            sid = kwargs["style_id"]
            if sid:
                style = self.env["mis.report.style"].browse(sid)
                if not style.exists():
                    raise UserError(_("Style with ID %s not found") % sid)
            values["style_id"] = sid if sid else False
        if kwargs.get("style_expression") is not None:
            values["style_expression"] = kwargs["style_expression"]
        return values
