...
class MisReportStyle(models.Model):
    ...
    def mis_style_update(
        ...
        values = self._build_style_update_values(
            name=name,
            color=color,
    ...
    def _validate_optional_choice(self, value, valid_options, field_name):
        """Validate an optional choice field value."""
        if value and value not in valid_options:
            raise UserError(
                _("%s must be one of: %s") % (field_name, ", ".join(valid_options))
            )

    def _build_style_update_values(self, **kwargs):
        """Build write values dict from optional style update kwargs."""
        values = {}
        for key in ("name", "indent_level", "divider", "hide_empty", "hide_always"):
            if kwargs.get(key) is not None:
                values[key] = kwargs[key]
        for key in ("color", "background_color", "prefix", "suffix"):
            if kwargs.get(key) is not None:
                values[key] = kwargs[key] if kwargs[key] else False
        if kwargs.get("decimal_places") is not None:
            values["dp"] = kwargs["decimal_places"]
        if kwargs.get("font_style") is not None:
            self._validate_optional_choice(
                kwargs["font_style"], ["normal", "italic"], "font_style"
            )
            values["font_style"] = (
                kwargs["font_style"] if kwargs["font_style"] else False
            )
        if kwargs.get("font_weight") is not None:
            self._validate_optional_choice(
                kwargs["font_weight"], ["normal", "bold"], "font_weight"
            )
            values["font_weight"] = (
                kwargs["font_weight"] if kwargs["font_weight"] else False
            )
        if kwargs.get("font_size") is not None:
            self._validate_optional_choice(
                kwargs["font_size"], ["medium", "small", "large"], "font_size"
            )
            values["font_size"] = kwargs["font_size"] if kwargs["font_size"] else False
        return values
