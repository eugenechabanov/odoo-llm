...
class MisReportInstancePeriod(models.Model):
    ...
    def mis_period_list(self, instance_id: int) -> dict:
        ...
        for period in instance.period_ids:
            period_data = {
                ...
                "source_display": dict(self._fields["source"].selection).get(
                    period.source
                ),
                "mode": period.mode,
            ...
            if period.mode == "fix":
                period_data["manual_date_from"] = (
                    str(period.manual_date_from) if period.manual_date_from else None
                )
                period_data["manual_date_to"] = (
                    str(period.manual_date_to) if period.manual_date_to else None
                )
            elif period.mode == "relative":
                period_data["type"] = period.type
                period_data["type_display"] = (
                    dict(self._fields["type"].selection).get(period.type)
                    if period.type
                    else None
                )
    ...
    def mis_period_create(
        ...
        if source in ["actuals", "actuals_alt"] and mode == "none":
            raise UserError(
                _("Actuals sources require a date filter (mode cannot be 'none')")
            )
        ...
        if mode == "relative":
            ...
            if not manual_date_from or not manual_date_to:
                raise UserError(
                    _("manual_date_from and manual_date_to are required for fixed mode")
                )
